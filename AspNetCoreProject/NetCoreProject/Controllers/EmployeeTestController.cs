...
namespace NetCoreProject.Controllers
{
    public class EmployeeTestController : Controller
    {
        ...
        {
            var httpClient = new HttpClient();
            var responseMessage =await httpClient.GetAsync("https://localhost:44358/api/Default/"+id);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var httpClient = new HttpClient();
            var responseMessage = await httpClient.DeleteAsync("https://localhost:44358/api/Default/" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
    ...
    // Ctrl+F5 ile api katmanını çalıştır. Daha sonra ctrl+Shift+W ile blog uygulamasını çalıştır. Video 120...
}
